...
from transformers import (MODEL_FOR_QUESTION_ANSWERING_MAPPING, WEIGHTS_NAME,
                          AdamW, AdapterConfig, AdapterType, AutoConfig,
...
def train(args, train_dataset, model, tokenizer, adapter_names=None):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training:
                        ...
                        if args.store_best_model and args.metric_score:
                            best_model_score, eval_steps_without_improvement = store_best_model(
                                model, tokenizer, args, results, best_model_score, eval_steps_without_improvement
                            )
            ...
            if (args.max_steps > 0 and global_step > args.max_steps) or eval_steps_without_improvement > (
                2000 / args.logging_steps
            ):
...
def store_best_model(model, tokenizer, args, eval_results, best_model_score, eval_steps_without_improvement):
    if eval_results[args.metric_score] > best_model_score:
        best_model_output_dir = os.path.join(args.output_dir, "best_model")
        if not os.path.exists(best_model_output_dir) and args.local_rank in [-1, 0]:
            os.makedirs(best_model_output_dir)

        logger.info("Saving best model to %s", best_model_output_dir)
        # Save a trained model, configuration and tokenizer using `save_pretrained()`.
        # They can then be reloaded using `from_pretrained()`
        # Take care of distributed/parallel training
        model_to_save = model.module if hasattr(model, "module") else model
        if args.train_adapter:
            model_to_save.save_all_adapters(best_model_output_dir)
        else:
            model_to_save.save_pretrained(best_model_output_dir)
        tokenizer.save_pretrained(best_model_output_dir)

        # Good practice: save your training arguments together with the trained model
        torch.save(args, os.path.join(best_model_output_dir, "training_args.bin"))

        with open(os.path.join(best_model_output_dir, "output.txt"), "w") as f:
            for key, value in eval_results.items():
                f.write("%s = %s\n" % (key, value))

        return eval_results[args.metric_score], 0
    eval_steps_without_improvement += 1
    logger.info("Number of eval steps without improvement: %d", eval_steps_without_improvement)
    if eval_steps_without_improvement > (2000 / args.logging_steps):
        logger.info(
            "%d eval steps have passed without score improvements. Early stopping training.",
            eval_steps_without_improvement,
        )
    return best_model_score, eval_steps_without_improvement
...
def main():
    ...
    parser.add_argument(
        "--metric_score", type=str, default=None, help="Metric used to determine best model.",
    )
    parser.add_argument(
        "--store_best_model", action="store_true", default=False, help="Whether to store best model.",
    )
    parser.add_argument(
        "--replace_embeddings", action="store_true", default=False, help="Whether to replace embeddings",
    )
    parser.add_argument(
        "--leave_out_twelvth", action="store_true", default=False, help="Whether to leave out adapters in layer 12"
    )
    parser.add_argument(
        "--is_japanese", action="store_true", default=False, help="Set to true when using Japanese model/tokenizer"
    )
    parser.add_argument(
        "--mecab_dir",
        type=str,
    ...
    if args.is_japanese:
        assert args.mecab_dir is not None
        assert args.mecab_dic_dir is not None
    tokenizer = AutoTokenizer.from_pretrained(
        args.tokenizer_name if args.tokenizer_name else args.model_name_or_path,
        do_lower_case=args.do_lower_case,
        cache_dir=args.cache_dir if args.cache_dir else None,
        mecab_kwargs={"mecab_option": f"-r {args.mecab_dir} -d {args.mecab_dic_dir}"} if args.is_japanese else None,
    )
    ...
    if args.replace_embeddings:
        model.resize_token_embeddings(len(tokenizer))

    if args.leave_out_twelvth:
        logger.info("Leaving out 12")
        leave_out = [11]
    else:
        leave_out = []

    setup_task_adapter_training(model, task_name, args, leave_out=leave_out, with_embeddings=args.replace_embeddings)
    if args.leave_out_twelvth:
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        if args.do_train:
            ...
            checkpoints = [os.path.join(args.output_dir, "best_model" if args.store_best_model else None)]
