...
from transformers import (AdapterArguments, AdapterType, AutoConfig,
                          ...
                          EvalPrediction, HfArgumentParser, PfeifferConfig,
...
class ModelArguments:
    ...
    do_lower_case: bool = field(default=False, metadata={"help": "Whether to use lower cased tokenization"})
...
def main():
    ...
    setup_task_adapter_training(model, task_name, adapter_args, with_embeddings=model_args.replace_embeddings)
                    ...
                    for line in f:
                        if line.startswith("-DOCSTART-") or line == "" or line == "\n":
                            ...
                            if not preds_list[example_id]:
                                example_id += 1
                                # logger.info("Starting new example after %s tokens", token_nr)
                                # token_nr = 0
                                # logger.info("Next example has sequence length: %s", len(preds_list[example_id]))
