...
import numpy as np
from transformers import (AdapterArguments, AdapterType, AutoConfig,
                          AutoModelForSequenceClassification, AutoTokenizer,
                          EvalPrediction, GlueDataset)
from transformers import GlueDataTrainingArguments as DataTrainingArguments
from transformers import (HfArgumentParser, PfeifferConfig, Trainer,
                          TrainingArguments, glue_compute_metrics,
...
def main():
    ...
    setup_task_adapter_training(
        model, task_name, adapter_args=adapter_args, with_embeddings=model_args.replace_embeddings
    )
    ...
    if training_args.do_predict:
        ...
        for test_dataset in test_datasets:
            predict_outputs = trainer.predict(test_dataset=test_dataset)
            predictions = predict_outputs.predictions
            test_result = predict_outputs.metrics

            if output_mode == "classification":
                predictions = np.argmax(predictions, axis=1)

            output_test_preds_file = os.path.join(
                training_args.output_dir, f"test_predictions_{test_dataset.args.task_name}.txt",
            )
            ...
            if trainer.is_world_master():
                ...
                with open(output_test_preds_file, "w", encoding="utf-8") as writer:
                    logger.info("***** Test predictions {} *****".format(test_dataset.args.task_name))
